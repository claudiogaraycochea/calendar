...
class WorkflowTemplateEditor extends Component {
  ...
  setWorkflowItemProperties = (valueId) => {
		console.log('setWorkflowItemProperties');
		const workflowData = [...this.state.workflowData];
		let workflowDataResult = [];
		const workflowItemSelectedId = this.state.workflowItemSelectedId;
		console.log('input: ',workflowData);
		/*for(let i=0; i<workflowData.length; i++){
			if(workflowData[i].id===workflowItemSelectedId) {
				console.log(workflowData[i]);
				console.log(workflowData[i].properties[0]);
				workflowData[i].properties[0].selected = valueId; 
			}
		}*/
		workflowData.map((item) => {
			if(item.id===workflowItemSelectedId) {
				console.log(item.id,' > ',valueId);
				item.properties[0].selected = valueId;
			}
			else {
				item.properties[0].selected = 3;
			}
			workflowDataResult.push(item);
		});
		
		
		console.log('output: ',workflowDataResult);
		/*this.setState({
